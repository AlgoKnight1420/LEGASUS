// Global variables
// Use a different name to avoid conflict with script.js
let productDetail = null; // Renamed from currentProduct
// Ensure we don't redeclare currentProduct if it's already defined
if (typeof currentProduct === 'undefined') {
    let currentProduct = null; // This will be used by script.js
}
let allProducts = [];
let cartItems = [];
let selectedSize = 'M';
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Shipping functionality has been removed
});
...
async function loadProductData() {
    ...
    if (storedProduct && storedAllProducts) {
        try {
            productDetail = JSON.parse(storedProduct);
            ...
            if (productId) {
                productDetail = allProducts.find(p => p.id == productId);
            }
            if (productDetail) {
                displayProduct(productDetail);
                displayRecommendedProducts();
...
function displayProduct(product) {
    ...
    if (isOutOfStock) {
        // Add out of stock badge to product image container
        const imageContainer = mainProductImage.parentElement;
        
        // Remove existing badge if any
        const existingBadge = imageContainer.querySelector('.out-of-stock-badge');
        if (existingBadge) {
        ...
        // Enable buttons if product is in stock
...
function displayRecommendedProducts() {
    if (!productDetail || allProducts.length === 0) return;
    
    // Get 4 random products excluding current product
    const otherProducts = allProducts.filter(p => p.id !== productDetail.id);
    ...
    recommendedProducts.innerHTML = recommended.map(product => {
        ...
        return `
            <div class="rec-card ${isOutOfStock ? 'out-of-stock-product' : ''}" onclick="navigateToProduct(${product.id})">
...
function addToCart() {
    ...
    const existingItem = cartItems.find(item => 
        item.id === productDetail.id && item.selectedSize === selectedSize
    );
    ...
    saveCartToStorage();
    updateCounters();
    showNotification(`${productDetail.name} (${selectedSize}) added to cart!`);
...
function openCheckoutModal() {
    ...
    const totalPrice = productDetail.price * selectedQuantity;
    
    checkoutItem.innerHTML = `
        <img src="${productDetail.image}" alt="${productDetail.name}">
        <div class="order-item-info">
            <div class="order-item-name">${productDetail.name}</div>
...
function openOrderSuccessModal() {
    ...
    // Create verification link from order ID
    const verificationLink = `verify-product.html?order=${orderId.substring(1)}`;
    
    // Display order ID as clickable link
    document.getElementById('orderId').innerHTML = `<a href="${verificationLink}" class="order-link" target="_blank">${orderId}</a>`;
...
function completeOrder() {
    ...
    if (paymentMethod === 'card' || paymentMethod === 'upi') {
        ...
        processRazorpayPayment(totalPrice, orderDetails);
    } else {
        // Cash on Delivery
        ...
        // Show success modal
        setTimeout(() => {
            document.getElementById('orderId').textContent = '#LEGASUS-' + Math.floor(100000 + Math.random() * 900000);
...
async function processRazorpayPayment(amount, orderDetails) {
    try {
        ...
        const options = {
            ...
            name: 'LEGASUS',
            description: 'Purchase from LEGASUS',
            order_id: data.id,
...
async function verifyPayment(paymentResponse, orderDetails) {
    try {
        ...
        showNotification('Payment verification failed: ' + error.message);
    }
}
